...
namespace Ndk
{
	...
	LuaBinding_Base(binding)
	{
		...
		stream.Reset("Stream");
		{
			...
			stream.BindMethod("Read", [] (Nz::LuaState& lua, Nz::Stream& byteStream, std::size_t /*argumentCount*/) -> int {
				int argIndex = 2;

				std::size_t length = lua.Check<std::size_t>(&argIndex);

				std::unique_ptr<char[]> buffer(new char[length]);
				std::size_t readLength = byteStream.Read(buffer.get(), length);

				lua.PushString(Nz::String(buffer.get(), readLength));
				return 1;
			});

			stream.BindMethod("Write", [] (Nz::LuaState& lua, Nz::Stream& byteStream, std::size_t /*argumentCount*/) -> int {
				int argIndex = 2;

				std::size_t bufferSize = 0;
				const char* buffer = lua.CheckString(argIndex, &bufferSize);

				if (byteStream.IsTextModeEnabled())
					lua.Push(byteStream.Write(Nz::String(buffer, bufferSize)));
				else
					lua.Push(byteStream.Write(buffer, bufferSize));
				return 1;
			});
		}
		...
		clock.Reset("Clock");
		{
			...
			// Manual
			clock.BindMethod("__tostring", [] (Nz::LuaState& lua, Nz::Clock& timeClock, std::size_t /*argumentCount*/) -> int {
				Nz::StringStream ss("Clock(Elapsed: ");
				ss << timeClock.GetSeconds();
				ss << "s, Paused: ";
				ss << timeClock.IsPaused();
				ss << ')';
		...
		file.Reset("File");
		{
			...
			// Manual
			file.BindMethod("Open", [] (Nz::LuaState& lua, Nz::File& dataFile, std::size_t argumentCount) -> int
			{
				std::size_t argCount = std::min<std::size_t>(argumentCount, 2U);
				...
				switch (argCount)
				{
					case 0:
					case 1:
						return lua.Push(dataFile.Open(lua.Check<Nz::UInt32>(&argIndex, Nz::OpenMode_NotOpen)));

					case 2:
					{
						Nz::String filePath = lua.Check<Nz::String>(&argIndex);
						Nz::UInt32 openMode = lua.Check<Nz::UInt32>(&argIndex, Nz::OpenMode_NotOpen);
						return lua.Push(dataFile.Open(filePath, openMode));
					}
				}
			...
			file.BindMethod("SetCursorPos", [] (Nz::LuaState& lua, Nz::File& dataFile, std::size_t argumentCount) -> int
			{
				std::size_t argCount = std::min<std::size_t>(argumentCount, 2U);
				...
				{
					case 1:
						return lua.Push(dataFile.SetCursorPos(lua.Check<Nz::UInt64>(&argIndex)));

					case 2:
					{
						Nz::CursorPosition curPos = lua.Check<Nz::CursorPosition>(&argIndex);
						Nz::Int64 offset = lua.Check<Nz::Int64>(&argIndex);
						return lua.Push(dataFile.SetCursorPos(curPos, offset));
					}
				}
			...
			file.BindMethod("__tostring", [] (Nz::LuaState& lua, Nz::File& dataFile, std::size_t /*argumentCount*/) -> int {
				Nz::StringStream ss("File(");
				if (dataFile.IsOpen())
					ss << "Path: " << dataFile.GetPath();
