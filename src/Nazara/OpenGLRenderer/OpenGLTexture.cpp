...
namespace Nz
{
	...
	Image OpenGLTexture::Download(TextureLayout /*textureLayout*/, UInt8 level)
	{
		...
		context.DownloadTexture(*this, level, resultImage.GetPixels());
	...
	void OpenGLTexture::Download(GpuAsyncCommands& /*asyncTransfer*/, TextureLayout textureLayout, std::function<void(Image&& resultImage)> callback, UInt8 level)
	{
		...
		Image resultImage = Download(textureLayout, level);
		callback(std::move(resultImage));
	}
	...
	bool OpenGLTexture::Update(const void* ptr, const Boxui& box, UInt32 srcWidth, UInt32 srcHeight, UInt8 level)
	{
		...
		UInt8 blockSize = PixelFormatInfo::IsUncompressed(m_textureInfo.pixelFormat) ? PixelFormatInfo::GetBytesPerPixel(m_textureInfo.pixelFormat) : PixelFormatInfo::GetBytesPerBlock(m_textureInfo.pixelFormat);
		if (blockSize % 8 == 0)
	...
	bool OpenGLTexture::Update(Nz::FunctionRef<bool(void* ptr)> callback, const Boxui& box, UInt8 level)
	{
		std::vector<std::uint8_t> pixels(PixelFormatInfo::ComputeSize(m_textureInfo.pixelFormat, box.width, box.height, box.depth));
		if (!callback(pixels.data()))
			return false;
