...
#if defined(NAZARA_PLATFORM_WINDOWS) || defined(NAZARA_PLATFORM_LINUX) || defined(NAZARA_PLATFORM_BSD) || defined(NAZARA_PLATFORM_ANDROID)
...
namespace Nz
{
	...
	std::unique_ptr<GL::Loader> OpenGLRenderer::SelectLoader(const Renderer::Config& config)
	{
		...
		{
			NazaraWarning("failed to load WGL: {0}", e.what());
		}
#endif
		...
		{
			NazaraWarning("failed to load EGL: {0}", e.what());
		}
#endif
		...
		{
			NazaraWarning("failed to load WebGL: {0}", e.what());
		}
#endif
