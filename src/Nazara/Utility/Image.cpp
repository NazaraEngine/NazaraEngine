...
namespace Nz
{
	...
	bool Image::Convert(PixelFormatType newFormat)
	{
		...
		for (unsigned int i = 0; i < levels.size(); ++i)
		{
			...
			levels[i].reset(new UInt8[pixelsPerFace * depth * NzPixelFormat::GetBytesPerPixel(newFormat)]);
	...
	void Image::Copy(const Image& source, const Boxui& srcBox, const Vector3ui& dstPos)
	{
		...
		UInt8 bpp = NzPixelFormat::GetBytesPerPixel(m_sharedImage->format);
	...
	bool Image::Create(ImageType type, PixelFormatType format, unsigned int width, unsigned int height, unsigned int depth, UInt8 levelCount)
	{
		...
		for (unsigned int i = 0; i < levelCount; ++i)
		{
			...
			try
			{
				levels[i].reset(new UInt8[w * h * d * NzPixelFormat::GetBytesPerPixel(format)]);
	...
	bool Image::Fill(const Color& color)
	{
		...
		if (!NzPixelFormat::Convert(PixelFormatType_RGBA8, m_sharedImage->format, &color.r, colorBuffer.get()))
		{
	...
	bool Image::FlipHorizontally()
	{
		...
		for (unsigned int level = 0; level < m_sharedImage->levels.size(); ++level)
		{
			...
			if (!NzPixelFormat::Flip(PixelFlipping_Horizontally, m_sharedImage->format, width, height, depth, ptr, ptr))
			{
	...
	bool Image::FlipVertically()
	{
		...
		{
			UInt8* ptr = m_sharedImage->levels[level].get();
			if (!NzPixelFormat::Flip(PixelFlipping_Vertically, m_sharedImage->format, width, height, depth, ptr, ptr))
			{
				NazaraError("Failed to flip image");
				return false;
			}
	...
	Color Image::GetPixelColor(unsigned int x, unsigned int y, unsigned int z) const
	{
		...
		const UInt8* pixel = GetPixelPtr(m_sharedImage->levels[0].get(), NzPixelFormat::GetBytesPerPixel(m_sharedImage->format), x, y, z, m_sharedImage->width, m_sharedImage->height);

		Color color;
		if (!NzPixelFormat::Convert(m_sharedImage->format, PixelFormatType_RGBA8, pixel, &color.r))
	...
	bool Image::SetPixelColor(const Color& color, unsigned int x, unsigned int y, unsigned int z)
	{
		...
		UInt8* pixel = GetPixelPtr(m_sharedImage->levels[0].get(), NzPixelFormat::GetBytesPerPixel(m_sharedImage->format), x, y, z, m_sharedImage->width, m_sharedImage->height);

		if (!NzPixelFormat::Convert(PixelFormatType_RGBA8, m_sharedImage->format, &color.r, pixel))
		{
