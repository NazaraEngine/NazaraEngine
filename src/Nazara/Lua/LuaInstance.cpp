...
namespace Nz
{
	...
	void LuaInstance::CheckType(int index, nzLuaType type) const
	{
		#ifdef NAZARA_DEBUG
	...
	bool LuaInstance::Compare(int index1, int index2, nzLuaComparison comparison) const
	{
		#ifdef NAZARA_DEBUG
	...
	void LuaInstance::Compute(nzLuaOperation operation)
	{
		#ifdef NAZARA_DEBUG
	...
	nzLuaType LuaInstance::GetField(const char* fieldName, int index) const
	{
		return FromLuaType(lua_getfield(m_state, index, fieldName));
	}

	nzLuaType LuaInstance::GetField(const String& fieldName, int index) const
	{
		return FromLuaType(lua_getfield(m_state, index, fieldName.GetConstBuffer()));
	}

	nzLuaType LuaInstance::GetGlobal(const char* name) const
	{
		return FromLuaType(lua_getglobal(m_state, name));
	}

	nzLuaType LuaInstance::GetGlobal(const String& name) const
	{
	...
	nzLuaType LuaInstance::GetMetatable(const char* tname) const
	{
		return FromLuaType(luaL_getmetatable(m_state, tname));
	}

	nzLuaType LuaInstance::GetMetatable(const String& tname) const
	{
	...
	nzLuaType LuaInstance::GetTable(int index) const
	{
	...
	nzLuaType LuaInstance::GetType(int index) const
	{
		return FromLuaType(lua_type(m_state, index));
	}

	const char* LuaInstance::GetTypeName(nzLuaType type) const
	{
		#ifdef NAZARA_DEBUG
		if (type > nzLuaType_Max)
		{
	...
	bool LuaInstance::IsOfType(int index, nzLuaType type) const
	{
		switch (type)
		{
			case nzLuaType_Boolean:
