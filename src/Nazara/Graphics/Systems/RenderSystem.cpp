...
namespace Nz
{
	...
	m_lightEntityPool(32)
	{
		...
		m_cameraObserver = m_world.observer<CameraComponent, NodeComponent>()
			.event(flecs::Monitor)
			.yield_existing()
			.each([&](flecs::iter& it, std::size_t i, CameraComponent& /*entityCamera*/, NodeComponent& /*entityNode*/)
			{
				...
				if (it.event() == flecs::OnAdd)
					m_pendingCameraEntities.insert(entity);
				else if (it.event() == flecs::OnRemove)
				{
					m_pendingCameraEntities.erase(entity);
		...
		m_graphicsObserver = m_world.observer<GraphicsComponent, NodeComponent>()
			.event(flecs::Monitor)
			.yield_existing()
			.each([&](flecs::iter& it, std::size_t i, GraphicsComponent& entityGfx, NodeComponent& /*entityNode*/)
			{
				flecs::entity entity = it.entity(i);
				if (it.event() == flecs::OnAdd)
					m_pendingGraphicsEntities.insert(entity);
				else if (it.event() == flecs::OnRemove)
				{
					m_pendingGraphicsEntities.erase(entity);
		...
		m_lightObserver = m_world.observer<LightComponent, NodeComponent>()
			.event(flecs::Monitor)
			.yield_existing()
			.each([&](flecs::iter& it, std::size_t i, LightComponent& entityLight, NodeComponent& /*entityNode*/)
			{
				flecs::entity entity = it.entity(i);
				if (it.event() == flecs::OnAdd)
					m_pendingLightEntities.insert(entity);
				else if (it.event() == flecs::OnRemove)
				{
					m_pendingLightEntities.erase(entity);
		...
		m_sharedSkeletonObserver = m_world.observer<GraphicsComponent, NodeComponent, SharedSkeletonComponent>()
		...
		m_skeletonObserver = m_world.observer<GraphicsComponent, NodeComponent, SkeletonComponent>()
	...
	{
		m_cameraObserver.destruct();
		m_graphicsObserver.destruct();
		m_lightObserver.destruct();
	...
	void RenderSystem::UpdatePending()
	{
		for (flecs::entity entity : m_pendingCameraEntities)
		{
			CameraComponent& entityCamera = entity.get_mut<CameraComponent>();
			NodeComponent& entityNode = entity.get_mut<NodeComponent>();
		...
		m_pendingCameraEntities.clear();

		for (flecs::entity entity : m_pendingGraphicsEntities)
		{
			GraphicsComponent& entityGfx = entity.get_mut<GraphicsComponent>();
			NodeComponent& entityNode = entity.get_mut<NodeComponent>();
		...
		m_pendingGraphicsEntities.clear();

		for (flecs::entity entity : m_pendingLightEntities)
		{
			LightComponent& entityLight = entity.get_mut<LightComponent>();
			NodeComponent& entityNode = entity.get_mut<NodeComponent>();
