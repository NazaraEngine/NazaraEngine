...
namespace Nz
{
	...
	*/
	void PhongLightingMaterial::Apply(const MaterialPipeline::Instance& instance) const
	{
		const Shader* shader = instance.renderPipeline.GetInfo().shader;
		const auto& shaderCache = GetShaderCache(shader);
		...
		auto GetShaderBindingPoint = [&](unsigned int binding)
		{
			auto it = shaderCache.bindings.find(binding);
			assert(it != shaderCache.bindings.end());
		...
		if (m_alphaMap)
		{
			unsigned int textureUnit = GetShaderBindingPoint(AlphaMapBinding);
		...
		if (m_diffuseMap)
		{
			unsigned int textureUnit = GetShaderBindingPoint(DiffuseMapBinding);
		...
		if (m_emissiveMap)
		{
			unsigned int textureUnit = GetShaderBindingPoint(EmissiveMapBinding);
		...
		if (m_heightMap)
		{
			unsigned int textureUnit = GetShaderBindingPoint(HeightMapBinding);
		...
		if (m_normalMap)
		{
			unsigned int textureUnit = GetShaderBindingPoint(NormalMapBinding);
		...
		if (m_specularMap)
		{
			unsigned int textureUnit = GetShaderBindingPoint(SpecularMapBinding);
	...
	*/
	...
	const PhongLightingMaterial::ShaderCache& PhongLightingMaterial::GetShaderCache(const Shader* shader) const
	{
		auto it = m_shaderCache.find(shader);
		if (it == m_shaderCache.end())
		{
			it = m_shaderCache.emplace(shader, ShaderCache()).first;

			ShaderCache& cache = it->second;
			cache.onDestroy.Connect(shader->OnShaderDestroy, this, &PhongLightingMaterial::InvalidateShader);
			cache.onUniformInvalidated.Connect(shader->OnShaderUniformInvalidated, this, &PhongLightingMaterial::InvalidateShader);

			cache.bindings = shader->ApplyLayout(s_renderPipelineLayout);
		}

		return it->second;
	}

	void PhongLightingMaterial::InvalidateShader(Shader* shader)
	{
		m_shaderCache.erase(shader);
	}
