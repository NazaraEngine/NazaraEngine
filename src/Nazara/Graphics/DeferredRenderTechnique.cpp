...
namespace Nz
{
	...
	bool DeferredRenderTechnique::IsSupported()
	{
		...
		return OpenGL::GetGLSLVersion() >= 140 && // On ne va pas s'embêter non plus avec le mode de compatibilité
			   Renderer::HasCapability(RendererCap_RenderTexture) &&
			   Renderer::HasCapability(RendererCap_MultipleRenderTargets) &&
			   Renderer::GetMaxColorAttachments() >= 4 &&
			   Renderer::GetMaxRenderTargets() >= 4;
	}
