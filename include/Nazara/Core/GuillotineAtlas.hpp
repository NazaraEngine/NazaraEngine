...
#ifndef NAZARA_CORE_GUILLOTINEATLAS_HPP
#define NAZARA_CORE_GUILLOTINEATLAS_HPP
...
namespace Nz
{
	class NAZARA_CORE_API GuillotineAtlas : public AbstractAtlas
	{
		public:
			inline GuillotineAtlas(PixelFormat pixelFormat, UInt32 initialLayerSize = 512);
			GuillotineAtlas(const GuillotineAtlas&) = delete;
			GuillotineAtlas(GuillotineAtlas&&) noexcept = default;
			~GuillotineAtlas() = default;
			...
			DataStoreFlags GetStorage() const override = 0;
			...
			GuillotineAtlas& operator=(const GuillotineAtlas&) = delete;
			GuillotineAtlas& operator=(GuillotineAtlas&&) noexcept = default;

		protected:
			struct Layer;

			virtual std::shared_ptr<AbstractImage> ResizeImage(const AbstractImage* oldImage, const Vector2ui& size) const = 0;
			virtual void UpdateImage(AbstractImage& image, const void* ptr, const Rectui& rect, UInt32 srcWidth, UInt32 srcHeight) const = 0;
...
#include <Nazara/Core/GuillotineAtlas.inl>

#endif // NAZARA_CORE_GUILLOTINEATLAS_HPP
